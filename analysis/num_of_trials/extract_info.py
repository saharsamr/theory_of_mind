    ...
    for i, common_reward in enumerate([0, 1]):
        for j, unique_reward in enumerate([0, 1]):
            ...
            data[i*2+j][2] = len(np.where(\
                common_reward_MF == common_reward and unique_reward_MF == unique_reward and repeat_MF == True\
            ))
            data[i*2+j][3] = len(np.where(\
                common_reward_MF == common_reward and unique_reward_MF == unique_reward and repeat_MF == False\
            ))
...
def groupby_data_single_subject_MB(common_reward_MB, reward_prob_MB, repeat_MB):
    ...
    for i, common_reward in enumerate([0, 1]):
        for j, reward_prob in enumerate(unique_reward_probs):
            ...
            data[i*2+j][2] = len(np.where(\
                common_reward_MB == common_reward and reward_prob_MB == reward_prob and repeat_MB == True
            ))
            data[i*2+j][2] = len(np.where(\
                common_reward_MB == common_reward and reward_prob_MB == reward_prob and repeat_MB == False
            ))
